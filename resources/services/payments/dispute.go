...
func Dispute() *schema.Table {
	return &schema.Table{
		Name:          "payments_dispute",
		Resolver:      fetchDispute,
		Transform:     transformers.TransformWithStruct(&payments.Dispute{}),
		IsIncremental: true,
...
func fetchDispute(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- interface{}) error {
	cl := meta.(*client.Client)

	key := fmt.Sprintf("payments-dispute-%s-%s", cl.Spec.WorkspaceId, cl.Spec.IntegrationAccountId)
	p := url.Values{}
	...
	cursor := fmt.Sprintf("%s/disputes", cl.Spec.Url)
	for {
		ret, p, err := cl.Services.GetDisputes(ctx, cursor, p)
		if err != nil {
			cl.Logger().Err(err)
